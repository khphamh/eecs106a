...
def main():
    ...
    print("Give start state joint angles")
    y = input()
    y = [float(c) for c in y.split()]
    goal.request.start_state.joint_state.position = [y[0], y[1], y[2], y[3], 0.0, 0.0, 0.0]
    ...
    print("Give end state joint angles")
    x = input()
    x = [float(c) for c in x.split()]
    target_joint_angles = [x[0], x[1], x[2], x[3], 0.0, 0.0, 0.0]
    tolerance = 0.0001
    consts = []
