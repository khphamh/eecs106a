...
def edge_detect_naive(gray_img):
    ...
    G_x = cv2.filter2D(src=blur, ddepth=-1, kernel=Kx)
    G_y = cv2.filter2D(src=blur, ddepth=-1, kernel=Ky)
    ...
    #G = np.sqrt(np.matmul(G_x, G_x) + np.matmul(G_y, G_y))
    
    G = G_x
    for i in range(np.shape(G_x)[0]):
        for j in range(np.shape(G_x)[1]):
            G[i][j] = ((G_x[i][j]**2 + G_y[i][j]**2))**0.5
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    d = 10
    img_d = cv2.resize(img, dsize=(img.shape[1]//d, img.shape[0]//d), interpolation=cv2.INTER_NEAREST)
    ...
    print(img_d.shape)
    img_d_copy = np.copy(img_d)
    img_d_copy = img_d_copy.transpose((2,0,1)).reshape(3,-1)
    img_d_copy = img_d_copy.T
    img_r = img_d_copy
...
def segment_image(img): 
    ...
    #binary = threshold_segment_naive(to_grayscale(img), 100, 150).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    test_img_2 = read_image(IMG_DIR + '/staples.jpg', grayscale=True)
    test_img_color = read_image(IMG_DIR + '/legos.jpg')

    # uncomment the test you want to run
    # it will plot the image and also save it

    #test_thresh_naive(test_img,100, 150)
    #test_edge_naive(test_img_2)
